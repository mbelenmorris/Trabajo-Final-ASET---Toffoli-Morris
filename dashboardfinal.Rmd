...
author: "María Magdalena Tóffoli y María Belén Morris"
output:
...
```{r}
#COn este código creamos el widget de shiny
# Interfaz de usuario
ui <- fluidPage(
    sidebarLayout(
    sidebarPanel(
      selectizeInput(
        inputId = "Variable",  # Cambié de "Categoria" a "Variable"
        label = "Selecciona la variable de agrupación:", 
    ...
    # Aquí se configura el espacio de la visualización
    mainPanel(
      plotlyOutput(outputId = "grafico", height = "600px", width = "100%") 
    )
  )
)
# Servidor
server <- function(input, output, ...) {
  output$grafico <- renderPlotly({
    req(input$Variable)  # Asegurar que no sea NULL
    ...
    datos_filtrados <- eph_filtrada2 %>% #datos filtrados nos sirve para luego construir el gráfico
      filter(!is.na(.data[[input$Variable]])) %>%  # Filtra filas con NA en la variable seleccionada
      group_by(ANO4, .data[[input$Variable]], Niveles_precariedad_total) %>%
      summarise(Niveles_precariedad_total_sum = sum(PONDERA, na.rm = TRUE), .groups = "drop") %>%
      group_by(ANO4, .data[[input$Variable]]) %>%
...
theme(axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = "bottom") +  # Mover la leyenda a la parte inferior
     #Rotar etiquetas del eje X si hay muchos años
      ...
      facet_wrap(~ .data[[input$Variable]], scales = "free_y") +  # Usar facet_wrap para desagregar por la variable seleccionada
...
```{r}
#Acá creamos un archivo en formato html
rmarkdown::render("dashboardfinal.Rmd", output_format = "html_document", output_file = "dashboardfinal.html")
